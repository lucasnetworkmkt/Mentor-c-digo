...
import { Play, Pause, RotateCcw, Edit3, Save } from 'lucide-react';
...
const ExecutionTimer: React.FC<ExecutionTimerProps> = ({ onComplete, timer, updateTimer }) => {
  ...
  useEffect(() => {
    if (timer.minutes === 0 && timer.seconds === 0 && !timer.isActive && timer.deliverable) {
       // Only show post check if it wasn't a manual reset (manual reset clears deliverable)
  ...
  const handleDeliveryConfirm = () => {
    // FIXO: 50 pontos por tarefa concluída, independente do modo (desde que haja entregável)
    const points = 50;
  ...
  // Calculate progress for visual circle (approximate based on current edit or default)
